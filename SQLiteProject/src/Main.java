...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String sql2 = "insert into g_artists (name, a_type, a_year, debut, regdate)" + " values ('방탄소년단','남성그룹','2010년대','2013sus',datetime('now', 'localtime'));";
			...
			String sql3 = "update g_artists set name = '마크튭' " + "where id = 2;";
			int cnt3 = stat3.executeUpdate(sql3);
			if(cnt3 > 0) {
				System.out.println("새로운 데이터가 수정되었습니다!");
			}
			else {
			...
			if(cnt4 > 0) {
				System.out.println("새로운 데이터가 삭제되었습니다!");
			}
			else {
